...
# визначення середніх планових цін продажу, возвращает pcvsodr_1 и pcvsodr_2
def pcvsodr_f (plan_df):
    if len(plan_df.loc[plan_df['pcvsogap']>=0]['pcvsogap'].values) == 0:
        return [plan_df.loc[plan_df['pcvsogap']==max(plan_df.loc[plan_df['pcvsogap']<0]['pcvsogap'].values)]['pcvapp'].values[0]]*2
    elif len(plan_df.loc[plan_df['pcvsogap']<0]['pcvsogap'].values) == 0:
        return [plan_df.loc[plan_df['pcvsogap']==min(plan_df.loc[plan_df['pcvsogap']>=0]['pcvsogap'].values)]['pcvapp'].values[0]]*2
    else:
        ...
        pcvsodr_1 = plan_df.loc[plan_df['pcvsogap']==pcvsogap_1]['pcvapp'].values[0]
        pcvsodr_2 = plan_df.loc[plan_df['pcvsogap']==pcvsogap_2]['pcvapp'].values[0]
...
# Поточна планова середня розрахована від рівня розпроданості
def DMCSOCPAP_f(stock_df,plan_df):
    # костіль т.к. должен быть "нулевой месяц"
    if max(pcvsop_f(plan_df)) == min(pcvsop_f(plan_df)):
        return min(pcvsodr_f(plan_df))
    else:
...
def DMWPDP_f(stock_df,config):
    DMWPDP = []
    DMWDPS = DMWDPS_f(stock_df, config)
    for i in range(0,len(stock_df)):
        DMWPDP.append(DMWDPS[i]*stock_df['scvdcp'][i])
    return DMWPDP
...
def DMWBHDP_f(stock_df,config):
    ...
    BMRS = BMRS_f(stock_df, config)
    for i in range(0,len(stock_df)):
        DMWBHDP.append(DMWDPS[i]*BMRS[i])
    return DMWBHDP
...
def DMPCBL_f(stock_df,plan_df,config):
    DMPCBL = []
    DMPMPPBL = DMPMPPBL_f(stock_df,plan_df,config)
    for i in range(0,len(stock_df)):
    ...
    return pd.DataFrame({'svid':list(stock_df['svid'].values), 'DMPCBL':DMPCBL, 'DMPMPPBL':DMPMPPBL})
